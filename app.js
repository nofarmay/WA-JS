...
function Binary(arr) {
  ...
  function findNextSquare(j) {
    return Math.sqrt(j) % 1 === 0 ? Math.pow(Math.sqrt(j) + 1, 2) : -1;
  }
  //
  //  Ex2.4 - Unique
  ...
  function YearsCenturies(year) {
    while (year % 100 === 0) {
      return year / 100;
    }
    syntaxxxx;
    element = Math.trunc(i / 100) + 1;
    return element;
  }
  ...
  // Examples
  // basicOp('+', 4, 7) // Output: 11basicOp('-', 15, 18) // Output: -3
  ...
  // clearly specified ;)Basic Iteration Logic
  ...
  //  function toWeirdCase(evenIndex,oddIndex) {
}
const text = "Hello world, it is such a weird ex.";
...
function maskify(userInput) {
  if (userInput.length > 4) {
    let cover = userInput.slice(0, -4);
    // console.log(cover);
    let uncover = userInput.slice(-4);
    // console.log(uncover);
    let newCover = cover.replaceAll(/a-zA-Z0-9/g, "#");
    let res = newCover + uncover;
    console.log(res);
  }
}
maskify("gtgtercde");
